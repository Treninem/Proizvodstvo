...
def create_request(chat_id: int, actor_user_id: int, values: dict[str, Any]) -> dict[str, Any]:
    ...
    with db.connect() as conn:
        ...
        conn.commit()
    for uid in repo.list_system_admin_ids(include_owner=True):
...
def get_request(chat_id:int, request_id:int, user_id:int)->dict[str,Any]|None:
    scope=_scope(chat_id)
    row=db.fetchone("""SELECT r.*,e.name AS entity_name,a.name AS area_name FROM replenishment_requests r JOIN entities e ON e.id=r.entity_id LEFT JOIN areas a ON a.id=r.area_id WHERE r.chat_id=? AND r.id=?""",(scope,int(request_id)))
...
def queue_forecast_notifications(now:datetime|None=None)->int:
    ...
    for scope in scopes:
        for item in forecast(scope, repo.list_system_admin_ids(include_owner=True)[0] if repo.list_system_admin_ids(include_owner=True) else 0):
