...
def main() -> None:
    ...
    assert "report:file:{token}:csv" not in download_block
    assert "report:file:{token}:html" not in download_block
    assert "report:file:{token}:txt" not in download_block
    ...
    assert not hasattr(reporting, "create_csv_report")
    assert not hasattr(reporting, "create_html_report")
    assert not hasattr(reporting, "create_txt_report")
    assert not hasattr(reporting, "create_universal_report_zip")
